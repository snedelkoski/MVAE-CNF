...
if __name__ == "__main__":
    ...
    # mode 2: generate conditioned on image
        ...
        if args.cuda:
            image  = image.cuda()
        mu, logvar = model.infer([image, None])
    ...
    # mode 3: generate conditioned on text
        ...
        mu, logvar = model.infer([None, text_repr])
        std        = logvar.mul(0.5).exp_()
    # mode 4: generate conditioned on image and text
        ...
        if args.cuda:
            image  = image.cuda()
            text_repr   = text_repr.cuda()
        mu, logvar = model.infer([image, text_repr])
        std        = logvar.mul(0.5).exp_()
    ...
    [img_recon, txt_recon] = model.decode(sample)
    img_recon = img_recon.cpu()
    txt_recon = txt_recon.cpu()
    ...
    with open('./sample_text.txt', 'w') as fp:
        txt_recon_np = txt_recon.data.numpy()
